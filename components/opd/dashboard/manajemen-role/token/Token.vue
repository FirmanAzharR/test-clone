<template>
        ...
        <template #no-data="{ row, index }">
          ...
          <template #name-data="{ row }">
            <div class="items-center justify-start max-w-xs gap-2">
              <span class="break-words whitespace-normal">{{ row.name }}</span>
            </div>
          </template>
          <template #description-data="{ row }">
            <div class="items-center justify-start max-w-xs gap-2">
              <span class="break-words whitespace-normal">{{ row.description }}</span>
            </div>
          </template>
          <template #app-data="{ row }">
            <div class="items-center justify-start max-w-xs gap-2">
              <span class="break-words whitespace-normal">{{ row.app.title }}</span>
            </div>
          </template>

          <template #publish-data="{ row }">
            <UBadge :color="row.publish === '1' ? 'emerald' : 'yellow'" variant="soft" class="px-2 py-1">
  ...
  <ModalAddEditToken :isOpen="isModalOpen" :mode="modalMode" :detailData="selectedDetail" @close="closeModal" />
</template>
...
import ModalAddEditToken from './modal/ModalAddEditToken.vue';
...
const openEditModal = async (id) => {
  ...
  try {
    await superAdminStore.getDetailToken(id);
    selectedDetail.value = superAdminStore.detailTokenData   || {};
    modalMode.value = 'edit';
    isModalOpen.value = true;
  } catch (err) {
    console.error('Error fetching token details:', err);
    toast.add({ title: 'Gagal mengambil data token', color: 'red' });
  } finally {
...
const handleDelete = async (id) => {
  layoutStore.setLoading(true);
  try {
    await superAdminStore.deleteToken(id);
...
const rows_table = computed(() => {
  return Array.isArray(superAdminStore.allDataToken?.data) ? superAdminStore.allDataToken.data : [];
});
...
const filteredRows = computed(() => {
  ...
  return rows_table.value.filter(row => {
    return (
      ...
      (row.instansi_id && row.instansi_id.toLowerCase().includes(query))
    );
  });
});
