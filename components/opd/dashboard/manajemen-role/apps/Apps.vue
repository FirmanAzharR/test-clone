<template>
  ...
  <ModalAddEditApps :isOpen="isModalOpen" :mode="modalMode" :detailData="selectedDetail" @close="closeModal" />
</template>
...
import ModalAddEditApps from './modal/ModalAddEditApps.vue';
...
const openEditModal = async (id) => {
  ...
  try {
    await superAdminStore.getDetailApps(id);
    selectedDetail.value = superAdminStore.detailAppsData   || {};
...
const handleDelete = async (id) => {
  layoutStore.setLoading(true);
  try {
    await superAdminStore.deleteApps(id);
...
{ key: 'no', label: 'No'},
  ...
  { key: 'instansi_id', label: 'ID Instansi', sortable: true },
...
const rows_table = computed(() => {
  return Array.isArray(superAdminStore.allDataApps?.data) ? superAdminStore.allDataApps.data : [];
});
...
onMounted(async () => {
  ...
  try {
    await superAdminStore.getAllApps();
    await superAdminStore.getOptionInstansi()
  } catch (error) {
