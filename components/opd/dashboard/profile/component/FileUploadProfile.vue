...
const props = defineProps({
  ...
  comment: {
    type: String,
    default: "Maximum image size is 350KB. Image dimensions must be 1420x900px pixels",
  },
});
...
const validateImageDimensions = (image) => {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      if (img.width === 1420 && img.height === 900) {
        resolve(true);
      } else {
        reject(new Error(`Dimensi gambar harus 1420x900 pixel. Dimensi saat ini: ${img.width}x${img.height}`));
      }
    };
...
const validateimageSize = (image) => {
  const maxSize = 350 * 1420; // 350KB
  if (image.size > maxSize) {
    throw new Error(`Ukuran image tidak boleh lebih dari 350KB. Ukuran saat ini: ${(image.size / 1420).toFixed(2)}KB`);
  }
  return true;
};
      ...
      <label :class="{
        ...
        <div v-if="image" class="w-full">
          <div class="flex items-center justify-between p-3 border rounded-lg">
            ...
            <button @click="removeimage" class="text-red-500 hover:text-red-900">
