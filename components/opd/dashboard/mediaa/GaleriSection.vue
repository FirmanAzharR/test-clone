...
import ModalAddEditGaleri from './modal/ModalAddEditGaleri.vue';
...
const openEditModal = async (id) => {
  ...
  try {
    await fotoStore.detailGaleri(id);
    selectedDetail.value = fotoStore.detailGaleriData || {};
    modalMode.value = 'edit';
    isModalOpen.value = true;
  } catch (err) {
    console.error('Error loading gallery details:', err);
    toast.add({ title: 'Gagal memuat detail galeri', color: 'red' });
  } finally {
...
const handleDelete = async (id) => {
  layoutStore.setLoading(true);
  try {
    await fotoStore.deleteGalleryAdmin(id);
...
const rows_table = computed(() => {
  return Array.isArray(fotoStore.allGaleriesData?.data) ? fotoStore.allGaleriesData.data : [];
});
...
onMounted(async () => {
  ...
  try {
    ...
    toast.add({ title: 'Gagal memuat data galeri', color: 'red' });
  } finally {
    loading.value = false;
  }
});
