        ...
        <template #no-data="{ row, index }">
          ...
          <template #title-data="{ row }">
            <div class="items-center justify-start max-w-md gap-2">
              ...
              <div v-if="row.descript" class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ truncateText(row.descript, 80) }}
              </div>
              <div v-if="row.link" class="mt-1 text-xs text-blue-500 truncate dark:text-blue-400">
                {{ row.link }}
              </div>
            </div>
          </template>
          ...
          <template #image-data="{ row }">
            <div class="h-24 overflow-hidden rounded-lg w-30">
...
import ModalAddEditSlider from './modal/ModalAddEditSlider.vue';
...
const openEditModal = async (id) => {
  ...
  try {
    await fotoStore.detailSliderAdmin(id);
    selectedDetail.value = fotoStore.detailSliderData || {};
    modalMode.value = 'edit';
    isModalOpen.value = true;
  } catch (err) {
    console.error('Error loading slider details:', err);
    toast.add({ title: 'Gagal memuat detail slider', color: 'red' });
  } finally {
...
const handleDelete = async (id) => {
  layoutStore.setLoading(true);
  try {
    await fotoStore.deleteSliderDataAdmin(id);
    if (fotoStore.deleteSliderData !== null) {
      await fotoStore.getAllSlider();
...
const rows_table = computed(() => {
  return Array.isArray(fotoStore.allSliderData?.data) ? fotoStore.allSliderData.data : [];
});
...
const filteredRows = computed(() => {
  ...
  return rows_table.value.filter(row => {
    return (
      ...
      (row.descript && row.descript.toLowerCase().includes(query)) ||
      (row.link && row.link.toLowerCase().includes(query))
    );
  });
});
...
onMounted(async () => {
  ...
  try {
    ...
    toast.add({ title: 'Gagal memuat data slider', color: 'red' });
  } finally {
    loading.value = false;
  }
});
