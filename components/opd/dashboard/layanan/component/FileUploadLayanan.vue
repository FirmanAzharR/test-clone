...
const props = defineProps({
  ...
  comment: {
    type: String,
    default: "Maximum image size is 200KB. Image dimensions must be 800x300px pixels",
  },
});
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      if (img.width === 800 && img.height === 300) {
        ...
        reject(new Error(`Dimensi gambar harus 800x300 pixel. Dimensi saat ini: ${img.width}x${img.height}`));
      }
    };
...
const validateimageSize = (image) => {
  const maxSize = 200 * 800; // 200KB
  if (image.size > maxSize) {
    throw new Error(`Ukuran image tidak boleh lebih dari 200KB. Ukuran saat ini: ${(image.size / 800).toFixed(2)}KB`);
  }
  return true;
};
