<template>
  ...
  <ModalAddEditLayanan :isOpen="isModalOpen" :mode="modalMode" :detailData="selectedDetail" @close="closeModal" />
</template>

<script setup>
import { ref, computed, onMounted, watch } from 'vue';
import { useBannerStore } from '~/store/banner/banner';
import ModalAddEditLayanan from './modal/ModalAddEditLayanan.vue';
...
const openEditModal = async (id) => {
  ...
  try {
    await bannerStore.detailLayananAdmin(id);
    selectedDetail.value = bannerStore.detailLayananData || {};
    modalMode.value = 'edit';
    isModalOpen.value = true;
  } catch (err) {
    console.error('Error fetching banner details:', err);
    toast.add({ title: 'Gagal mengambil data banner', color: 'red' });
  } finally {
...
const handleDelete = async (id) => {
  try {
    await bannerStore.deleteLayananAdmin(id);
    await bannerStore.getAllBanner();
...
const rows_table = computed(() => {
  return Array.isArray(bannerStore.allBannerData?.data) ? bannerStore.allBannerData.data : [];
});
...
onMounted(async () => {
  ...
  try {
    ...
    toast.add({ title: 'Gagal memuat data banner', color: 'red' });
  } finally {
    loading.value = false;
  }
});
