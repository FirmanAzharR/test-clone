...
const props = defineProps({
  ...
  comment: {
    type: String,
    default: "Maximum image size is 300KB. Image dimensions must be 1024x700 pixels",
  },
  required: {
    type: Boolean,
    default: false, // Added this prop to control if the field is required
  }
});
...
const validateimageSize = (image) => {
  ...
  if (image.size > maxSize) {
    throw new Error(`Ukuran image tidak boleh lebih dari 300KB. Ukuran saat ini: ${(image.size / 1024).toFixed(2)}KB`);
  }
  return true;
};
...
const handleimage = async (event) => {
  ...
  // If no image is selected and the field is not required, clear the field without error
  if (!selectedimage) {
    removeimage();
    return;
  }
    ...
    <div class="h-fit">
      <label :class="{
        ...
        <div v-else class="text-center">
          ...
          <p v-if="!required" class="mt-1 text-xs text-gray-400">(Optional)</p>
        </div>
