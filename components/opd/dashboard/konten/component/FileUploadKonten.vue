...
const props = defineProps({
  ...
  comment: {
    type: String,
    default:
      'Maximum image size is 350KB. Image dimensions must be 1024x700 pixels',
  },
});
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      if (img.width === 1024 && img.height === 700) {
        ...
        reject(
          new Error(
            `Dimensi gambar harus 1024x700 pixel. Dimensi saat ini: ${img.width}x${img.height}`
          )
        );
      }
    };
...
const validateimageSize = (image) => {
  ...
  if (image.size > maxSize) {
    throw new Error(
      `Ukuran image tidak boleh lebih dari 350KB. Ukuran saat ini: ${(
        image.size / 1024
      ).toFixed(2)}KB`
    );
  }
...
<template>
  <div class="relative my-3" :class="{ success: meta.valid }">
    <label
      v-if="label"
      :for="name"
      class="block mb-2 text-sm dark:text-white md:text-base"
    ...
    <div class="h-fit">
      ...
      >
        ...
        <div v-else class="text-center">
          ...
          />
          <div class="mt-2">
            <span class="ml-1 text-sm font-semibold text-blue-500"
              >Browse a Image</span
            >
          </div>
