  ...
  <div class="relative min-h-screen bg-slate-100 dark:bg-slate-900">
      ...
      <div class="absolute bottom-0 left-0 w-full p-6 md:p-8 lg:p-10">
        <div class="container mx-auto max-w-7xl">
          <div class="inline-block px-3 py-1 mb-3 text-sm font-medium text-white rounded-full bg-gradient-to-r from-blue-500 to-blue-400 backdrop-blur-sm">
            {{ detailKategori || 'Artikel' }}
          </div>
          
          <h1 class="max-w-4xl mb-2 text-2xl font-bold text-white md:text-3xl lg:text-4xl line-clamp-3">
            {{ detailTitle || 'Detail Berita & Informasi Terkini Dinas Komunikasi dan Informatika DIY' }}
          </h1>
          
          <div class="flex flex-wrap items-center gap-4 mt-4">
            <div class="flex items-center text-blue-100">
              <UIcon name="i-heroicons-calendar" class="w-4 h-4 mr-2" />
              <span class="text-sm">{{ formatDate(detailDate || new Date()) }}</span>
            </div>

            <div class="flex items-center text-blue-100">
              <UIcon name="i-heroicons-user-circle" class="w-4 h-4 mr-2" />
              <span class="text-sm">{{ detailCreator || 'Admin Kominfo' }}</span>
            </div>

            <div class="flex items-center text-blue-100">
              <UIcon name="i-heroicons-eye" class="w-4 h-4 mr-2" />
              <span class="text-sm">{{ detailVisitor || Math.floor(Math.random() * 1000) + 500 }} views</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="container px-4 mx-auto max-w-7xl pt-6 pb-6">
      <div class="py-4">
        <Breadcrumb 
          :items="[
            { text: selectedCategory || 'Semua Kategori', to: `/${(selectedCategory === 'Article' ? 'artikel' : selectedCategory === 'Berita' ? 'berita' : 'Informasi')}` },
            { text: detailTitle || 'Detail Artikel', to: '' }
          ]"
      ...
      <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2">
          <div class="p-6 bg-white shadow-xl rounded-2xl dark:bg-slate-800 md:p-8">
            <!-- Article content -->
            <div class="prose max-w-none dark:prose-invert prose-blue prose-img:rounded-xl prose-headings:font-bold prose-a:text-blue-600">
              <time class="text-primary-600">{{ formatDate(detailDate) }}</time>
              
              <p class="text-lg font-medium text-slate-700 dark:text-slate-300">
                {{ detailTitle || 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam auctor, nisl eget ultricies tincidunt, nisl nisl aliquam nisl, eget ultricies nisl nisl eget nisl. Sed vitae enim eget nisl aliquam aliquam. Sed vitae enim eget nisl aliquam aliquam. Sed vitae enim eget nisl aliquam aliquam. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam auctor, nisl eget ultricies tincidunt, nisl nisl aliquam nisl, eget ultricies nisl nisl eget nisl. Sed vitae enim eget nisl aliquam aliquam. Sed vitae enim eget nisl aliquam aliquam. Sed vitae enim eget nisl aliquam aliquam.' }}
              </p>
            </div>                  
            <!-- Article content with HTML -->
            <div class="prose max-w-none overflow-hidden [&_*:not([style*='color'])]:!text-slate-700 dark:[&_*:not([style*='color'])]:!text-slate-300 [&>ul>li::marker]:text-slate-700 dark:[&>ul>li::marker]:text-slate-300 [&>ol>li::marker]:text-slate-700 dark:[&>ol>li::marker]:text-slate-300 [&_a]:hover:!text-blue-600 dark:[&_a]:hover:!text-blue-400 [&_table]:!bg-white dark:[&_table]:!bg-slate-800 [&_tr]:!bg-white dark:[&_tr]:!bg-slate-800 [&_th]:!bg-slate-100 dark:[&_th]:!bg-slate-700 [&_td]:!bg-white dark:[&_td]:!bg-slate-800 content-wrapper" v-html="detailDescript"></div>
            <!-- Share buttons -->
        ...
        <div class="lg:col-span-1">
          ...
          <div class="p-6 bg-white shadow-lg rounded-2xl dark:bg-slate-800 md:p-6">
            <div class="flex items-center justify-between mb-6">
              <h2 class="text-xl font-bold text-slate-800 dark:text-white">Berita Terpopuler</h2>
            ...
            <div class="space-y-4">
              <div v-for="(item, index) in berita" :key="index" 
                ...
                <div class="flex-shrink-0 w-20 h-20 overflow-hidden rounded-lg">
                  ...
                  <template v-else>
                    <div class="flex flex-col items-center justify-center w-full h-full bg-gray-100 dark:bg-gray-900">
                ...
                <div class="flex-1">
                  <h3 class="font-medium text-slate-800 dark:text-white line-clamp-2">{{ item.title }}</h3>
          ...
          <div class="p-6 mt-6 bg-white shadow-lg rounded-2xl dark:bg-slate-800 md:p-6">
            <div class="flex items-center justify-between mb-6">
              <h2 class="text-xl font-bold text-slate-800 dark:text-white">Artikel Terpopuler</h2>
              <!-- {{ beritaStore.detailBeritaData }} -->
              <div class="w-10 h-1 rounded-full bg-gradient-to-r from-blue-500 to-blue-400"></div>
            </div>
            
            <div class="space-y-4">
              <div v-for="(item, index) in artikel" :key="index" 
                ...
                <div class="flex items-center justify-between mb-3">
                  <div class="px-2 py-1 text-xs font-medium text-blue-700 bg-blue-100 rounded-full dark:bg-blue-900/50 dark:text-blue-300">
                    {{ item.kategori }}
                  </div>
                ...
                <template v-if="isEmbeddedContent(item.description)">
                  <div class="mt-2 text-sm text-blue-600 dark:text-blue-400">
                    <UIcon name="i-heroicons-document" class="w-4 h-4 inline-block mr-1" />
                    <span>File Terlampir</span>
                  </div>
                </template>
                <template v-else>
                  <p class="mt-2 text-sm text-slate-600 dark:text-slate-400 line-clamp-2" v-html="item.description"></p>
                </template>
...
const fetchData = async () => {
  try {
    ...
    if (beritaStore.artikelData.data && beritaStore.artikelData.data.length > 0) {
      artikel.value = beritaStore.artikelData.data;
    }
    
    // Fetch events using store
    await beritaStore.getKegiatanData()
    if (beritaStore.kegiatanData.data && beritaStore.kegiatanData.data.length > 0) {
...
const isEmbeddedContent = (content) => {
  ...
  return contentStr.includes('iframe') || 
