<template>
  ...
  >
    ...
    <div class="container px-4 mx-auto max-w-7xl pt-20 pb-2">
      <div class="py-2">
        <Breadcrumb :items="[{ text: 'Kebijakan Privasi', to: '' }]" :loading="pending" />
      </div>
    </div>
...
// Fetch kebijakan privasi data
const { baseUrl } = await $fetch('/api/otp/getBaseUrl')
const { tokenInAuth } = await $fetch('/api/getEnv/getTokenEnv')
const { data, pending, error, refresh } = await useFetch(
  () => `${apiBaseUrl}/api/page/page/kebijakan-privasi`,
  {
    headers: {
